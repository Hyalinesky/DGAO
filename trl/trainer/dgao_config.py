...
@dataclass
class DGAOConfig(TrainingArguments):
    ...
    model_init_kwargs: Optional[Union[dict, str]] = field(
        ...
        metadata={
            ...
            "argument of the `DGAOTrainer` is provided as a string."
        },
    )
    ...
    # The default value remove_unused_columns is overwritten from the parent class, because in DGAO we usually rely on
    ...
    num_orders: Optional[int] = field(
        default=None,
        metadata={
            "help": "Number of prompt order variants per group for order-fair training. If specified, this will "
            "override `num_generations`. This parameter is specifically for training order-fair models where each "
            "prompt group contains multiple variants with different ordering."
        },
    )
    ...
    delta: Optional[float] = field(
        default=None,
        metadata={
            "help": "Enables the upper clipping bound in two-sided DGAO loss when set to a float. If `None` "
            "(default), standard DGAO clipping is used. Recommended to be greater than `1 + ε` when enabled. This "
    ...
    scale_rewards: bool = field(
        ...
        metadata={
            ...
            "scaling is applied. The Dr. DGAO paper recommends not scaling the rewards, as scaling by the standard "
    ...
    def __post_init__(self):
        ...
        # Handle num_orders parameter - if provided, it overrides num_generations
        if self.num_orders is not None:
            if self.num_generations != 8:  # 8 is the default value
                print(f"Warning: Both num_orders ({self.num_orders}) and num_generations ({self.num_generations}) "
                      f"are specified. Using num_orders and ignoring num_generations.")
            self.num_generations = self.num_orders
        ...
        if self.num_generations < 2:
            param_name = "num_orders" if self.num_orders is not None else "num_generations"
            raise ValueError(
                f"DGAO requires at least 2 generations per prompt to calculate the advantages. You provided "
                f"{param_name}={self.num_generations}, which is less than the minimum required."
            )
        possible_values = [
        ...
        #         f"prompt ({param_name}={self.num_generations}). Given the current effective train batch size, the valid values for "
        ...
        if self.eval_strategy != "no":
            ...
            if self.num_generations not in possible_values:
                ...
                raise ValueError(
                    ...
                    f"evenly divisible by the number of generations per prompt ({param_name}={self.num_generations}). Given the "
        ...
        if self.delta is not None and self.use_liger_loss:
            raise ValueError("Liger loss does not support two-sided DGAO loss yet.")