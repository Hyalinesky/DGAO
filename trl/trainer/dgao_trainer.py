...
from .dgao_config import DGAOConfig
from .utils import (
...
class DGAOTrainer(Trainer):
    """
    Modified DGAOTrainer for training order-fair LLMs.
    
    Example:

    ```python
    from datasets import load_dataset
    from trl import DGAOTrainer
    ...
    Args:
        model (`Union[str, PreTrainedModel]`):
            Model to be trained.
        args ([`GRPO`], *optional*, defaults to `None`):
    ...
    def __init__(
        ...
        if args is None:
            ...
            args = DGAO(f"{model_name}-OrderFair-DGAO")
        ...
        if isinstance(model, str):
            ...
            if isinstance(torch_dtype, torch.dtype) or torch_dtype == "auto" or torch_dtype is None:
                ...
                raise ValueError(
                    "Invalid `torch_dtype` passed to `DGAO`. Expected either 'auto' or a string representing "
            ...
            if args.model_init_kwargs is not None:
                raise ValueError(
                    "You passed `model_init_kwargs` to the `DGAO`, but your model is already instantiated."
                )
        ...
        if (
            ...
            raise NotImplementedError(
                "Iterable datasets are not yet supported in DGAOTrainer. Please use a standard dataset instead."
            )
    ...
    def _calculate_hybrid_advantages(self, rewards, num_groups):
        ...
        all_group_rewards = self.accelerator.gather(group_rewards)
    ...
    def _enable_gradient_checkpointing(self, model: PreTrainedModel, args: DGAOConfig) -> PreTrainedModel:
